...
(function($, window, undefined) {
    ...
    $.extend(neonChat, {
        ...
        init: function() {
            ...
            socket.on('user_names', function(data) {
                ...
                for (var i = 0; i < data.length; i++) {
                    // group_id, display_name, status, prepend, user_id
                    // if ($('#chat-user-' + data[i]).length == 0) {
                    //     neonChat.addUser('group-social', data[i], 'online', 1, 'chat-user-' + data[i]);
                    // };
            ...
            socket.on('new_message', function(data) {
                // console.log('from ' + data.sendfrom);
                // console.log('to ' + data.sendto);
                // console.log('msg ' + data.msg);
                // console.log('len ' + $('ul#chat_history_' + data.sendfrom).length)
                console.log(data)
                if ($('ul#chat_history_' + data.sendfrom).length == 0) {
                    console.log('not existed');
                    $('div#chat-history').append('<ul class="chat-history" id="chat_history_' + data.sendfrom + '"></ul>');
                };
                $('ul#chat_history_' + data.sendfrom).append('<li class="opponent unread odd"><span class="user">' + data.sendfrom + '</span><p>' + data.msg + '</p><span class="time">' + data.time + '</span></li>');
                var user_link = $('a#chat-user-' + data.sendfrom);
                // console.log('renid ' + id);
                // neonChat.renderMessages(id);
                // neonChat.refreshUserIds();
                // neonChat.orderGroups();
                // neonChat.renderMessages(user_link.attr('id'));
                neonChat.prefetchMessages();
                // neonChat.puffUnreads();
                neonChat.updateScrollbars();
                // neonChat.updateConversationOffset(user_link);
                $textarea.focus();
                var current_user = user_link;
                // neonChat.isOpen = true;
                neonChat.renderMessages(user_link.attr('id'));
                neonChat.resetUnreads(user_link.attr('id'));
                // neonChat.prefetchMessages();
        ...
        open: function($user_link) {
            ...
            this.isOpen = true;
            console.log("id " + this.$current_user.attr('id'))
        ...
        submitMessage: function() // Submit whats on textarea
            {
                ...
                if (this.isOpen && this.$current_user) {
                    ...
                    // var user = id.replace(/chat-user-/g, '');
                    // console.log('send ' + id);
                    // var currentdate = new Date();
                    // var time = currentdate.getHours() + ":" + currentdate.getMinutes();
                    // socket.emit('send_message', msg, user, time);
        ...
        renderMessages: function(id, slient) {
            if (typeof this.chat_history[id] != 'undefined') {
                ...
                // console.log('mess :' + this.chat_history[id].messages[i])
                for (var i in this.chat_history[id].messages) {
                    // console.log('mess :' + this.chat_history[id].messages[i])
                    var entry = this.chat_history[id].messages[i],
                        // $entry = $('<li><div class="conver-inner"><span class="user"></span><p></p><span class="time"></span></div></li>'),
                    ...
                    if (entry.unread && typeof slient == 'undefined') {
                        $entry.find('div.conver-inner').addClass('unread');
